...
for root, dirs, files in os.walk(setpath):
    ...
    if keyWord in root:
        ...
        for f in files:
            index = files.index(f)
            if length == 6:               
                replacement = os.path.splitext(f)[0] + "_" + orderLen6[index] +".jpg"                
            elif length == 8:
                replacement = os.path.splitext(f)[0] + "_" + orderLen8[index] +".jpg"                
            try:
                # rename picture according to orientation
                os.rename(f, replacement)
                f = replacement
            except:
                print("We don't know orientation of pictures!")
